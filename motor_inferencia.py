...
"""
MODIFICAR LA SECCION DE self.preguntas_display DE
...
class MotorInferenciaHardware:
    ...
    def obtener_siguiente_pregunta(self, clave_actual, respuesta):
        try:
            ...
            # NUEVO: Preguntas que NO deben mostrarse al usuario
            preguntas_internas = [
                'ejecutar_diagnostico', 
                'fin_diagnostico', 
                'fin_arbol',
                'sintoma_actual_mas_especifico'
            ]
            ...
            if resultado and len(resultado) > 0:
                ...
                if siguiente:
                    siguiente_str = str(siguiente)
                    
                    if siguiente_str in preguntas_internas:
                        print(f"  [DEBUG] Pregunta interna detectada: {siguiente_str} - Finalizando flujo")
                        return None
    ...
    def mejorar_nombre_diagnostico(self, nombre):
        mejoras = {
            ...
            'diagnostico_incompleto_pocos_sintomas': 'Diagnóstico Incompleto',

            # NIVEL 1: DIAGNÓSTICOS CRÍTICOS DE EMERGENCIA
            'componente_quemado_critico': 'Componente Quemado Crítico',
            'fallo_electrico_severo': 'Fallo Eléctrico Severo',
            'cortocircuito_detectado': 'Cortocircuito Detectado',
            'dano_por_tormenta_electrica': 'Daño por Tormenta Eléctrica',
            'dano_liquido_severo': 'Daño Severo por Líquido',
            'dano_impacto_fisico': 'Daño por Impacto Físico',

            # NIVEL 2: SISTEMA NO ENCIENDE (SIN SIGNOS DE VIDA)
            'psu_o_motherboard_muerta_simple': 'PSU o Motherboard Muerta',
            'problema_boton_power': 'Problema Botón Power',
            'psu_proteccion_activa': 'Protección PSU Activada',

            # NIVEL 3: SISTEMA ENCIENDE PERO NO POST (SIN VIDEO)
            'problema_post_sin_video': 'No Pasa POST - Sin Video',
            'problema_ram_no_post': 'Problema RAM en POST',
            'problema_gpu_no_detectada': 'GPU No Detectada',
            'problema_cpu_no_detectado': 'CPU No Detectado',

            # NIVEL 4: SISTEMA ENCIENDE CON VIDEO PERO PROBLEMAS
            'problema_arranque_lento': 'Arranque Extremadamente Lento',
            'problema_pantalla_azul_bsod': 'Pantalla Azul BSOD',
            'problema_artefactos_visuales': 'Artefactos Visuales en Pantalla',

            # NIVEL 5: PROBLEMAS DE ESTABILIDAD
            'problema_reinicios_aleatorios_general': 'Reinicios Aleatorios',
            'problema_congelamientos_general': 'Sistema se Congela',
            'problema_aplicaciones_crash': 'Aplicaciones Crash',

            # NIVEL 6: PROBLEMAS TÉRMICOS
            'problema_sobrecalentamiento_general': 'Sobrecalentamiento General',
            'problema_apagado_termico': 'Apagado por Protección Térmica',
            'problema_ventiladores_ruido': 'Ventiladores Ruidosos',
            'problema_ventilador_detenido': 'Ventilador Detenido',

            # NIVEL 7: PROBLEMAS DE STORAGE
            'problema_disco_no_detectado': 'Disco No Detectado',
            'problema_ruidos_disco': 'Disco Hace Ruidos Extraños',
            'problema_smart_warnings': 'Alertas SMART del Disco',
            'problema_archivos_corruptos': 'Archivos Corruptos',

            # NIVEL 8: PROBLEMAS DE RED Y CONECTIVIDAD
            'problema_red_no_funciona': 'Red No Funciona',
            'problema_wifi_no_detecta': 'WiFi No Detecta Redes',
            'problema_velocidad_red_baja': 'Velocidad Red Muy Baja',

            # NIVEL 9: PROBLEMAS USB Y PERIFÉRICOS
            'problema_usb_no_funciona': 'USB No Funciona',
            'problema_usb_intermitente': 'USB Intermitente',

            # NIVEL 10: PROBLEMAS DE AUDIO
            'problema_audio_no_funciona': 'Audio No Funciona',
            'problema_audio_crackling': 'Audio con Interferencias',

            # NIVEL 11: PROBLEMAS POST-INSTALACIÓN
            'problema_post_instalar_hardware': 'Problemas Post-Instalación Hardware',
            'problema_post_actualizar_drivers': 'Problemas Post-Actualización Drivers',
            'problema_post_actualizar_bios': 'Problemas Post-Actualización BIOS',

            # NIVEL 12: CATCH-ALL INTERMEDIO
            'problema_hardware_no_especificado': 'Problema Hardware No Especificado',
            'sistema_funcional_con_sintomas_menores': 'Sistema Funcional con Síntomas Menores',
            'diagnostico_requiere_mas_informacion': 'Requiere Más Información',

            # NIVEL 13: PROBLEMAS ESPECÍFICOS AVANZADOS
            'problema_dual_channel_ram_no_activo': 'Dual Channel RAM No Activo',
            'problema_xmp_no_activado': 'Perfil XMP No Activado',
            'problema_bios_desactualizada': 'BIOS Desactualizada',
            'problema_cmos_battery_low': 'Batería CMOS Baja',
            'problema_gpu_throttling_temperatura': 'GPU Throttling por Temperatura',
            'problema_cpu_throttling_temperatura': 'CPU Throttling por Temperatura',
            'problema_ssd_trim_deshabilitado': 'TRIM SSD Deshabilitado',
            'problema_disco_lleno_performance': 'Disco Lleno - Performance Degradada',
            'problema_overclock_inestable': 'Overclock Inestable',
            'problema_psu_capacidad_insuficiente': 'PSU Capacidad Insuficiente',
            'problema_polvo_acumulado_general': 'Polvo Acumulado Excesivo',
            'problema_pasta_termica_vieja': 'Pasta Térmica Seca/Vieja',
            'problema_driver_gpu_viejo': 'Drivers GPU Desactualizados',
            'problema_conflicto_drivers': 'Conflicto Entre Drivers',
            'problema_malware_performance': 'Malware Consume Recursos',

            # NIVEL 14: SOFTWARE Y SISTEMA OPERATIVO
            'problema_windows_corrupto': 'Archivos Windows Corruptos',
            'problema_fast_boot_causa_problemas': 'Fast Boot Causa Problemas',

            # NIVEL 15: CASOS EDGE Y SÍNTOMAS ÚNICOS
            'problema_coil_whine_psu_gpu': 'Coil Whine PSU/GPU',
            'problema_screen_tearing': 'Screen Tearing',
            'problema_no_signal_monitor': 'Monitor No Signal',
            'problema_resolucion_incorrecta': 'Resolución Incorrecta',
            'problema_colores_distorsionados': 'Colores Distorsionados',
            'problema_parpadeo_pantalla': 'Pantalla Parpadea',
            'problema_microfono_no_funciona': 'Micrófono No Funciona',
            'problema_nivel_audio_bajo': 'Nivel Audio Muy Bajo',

            # NIVEL 16: CATCH-ALL ABSOLUTO FINAL
            'problema_detectado_consultar_tecnico': 'Requiere Evaluación Técnica Profesional',

            # ===== DIAGNÓSTICOS GENERALES (YA EXISTENTES) =====
            'no_faults_detected': 'Sistema Funcionando Correctamente',
            'insufficient_symptoms': 'Síntomas Insuficientes para Diagnóstico',
            'problema_general_hardware': 'Problema General de Hardware',
            'diagnostico_incompleto': 'Diagnóstico Incompleto',
            'diagnostico_incompleto_pocos_sintomas': 'Diagnóstico Incompleto - Pocos Síntomas',
