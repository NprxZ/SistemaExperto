...
class DialogoInformeDiagnostico:
    ...
    def _on_mousewheel(self, event):
            ...
            try:
                # Linux usa Button-4 (scroll up) y Button-5 (scroll down)
                if event.num == 5 or event.delta < 0:
...
class DialogoHistorial:
    ...
    def mostrar(self):
        ...
        self.lista_canvas.bind_all("<MouseWheel>", self._on_mousewheel_lista)
        self.lista_canvas.bind_all("<Button-4>", self._on_mousewheel_lista)
        self.lista_canvas.bind_all("<Button-5>", self._on_mousewheel_lista)
        ...
        self.detalles_canvas.bind("<MouseWheel>", self._on_mousewheel_detalles)
        self.detalles_canvas.bind("<Button-4>", self._on_mousewheel_detalles)
        self.detalles_canvas.bind("<Button-5>", self._on_mousewheel_detalles)
    ...
    def _on_mousewheel_lista(self, event):
        try:
            if not self.lista_canvas.winfo_exists():
                return
        except (AttributeError, tk.TclError):
            return
        ...
        try:
            if event.num == 5 or event.delta < 0:
                self.lista_canvas.yview_scroll(1, "units")
            elif event.num == 4 or event.delta > 0:
                self.lista_canvas.yview_scroll(-1, "units")
        except tk.TclError:
            pass
        except Exception as e:
            print(f"Error en mousewheel lista: {e}")

    def _on_mousewheel_detalles(self, event):
        try:
            if not self.detalles_canvas.winfo_exists():
                return
        except (AttributeError, tk.TclError):
            return
        ...
        try:
            if event.num == 5 or event.delta < 0:
                self.detalles_canvas.yview_scroll(1, "units")
            elif event.num == 4 or event.delta > 0:
                self.detalles_canvas.yview_scroll(-1, "units")
        except tk.TclError:
            pass
        except Exception as e:
            print(f"Error en mousewheel detalles: {e}")
