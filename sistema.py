...
"""
MODIFICAR LA SECCION DE PREGUNTAS/CATEGORIA DE
ESTE ARCHIVO SI DECIDIMOS AGRAGR MAS PREGUNTAS EN EL ARCHIVO .PL
...
class AsistenteFlotante:
    def __init__(self, canvas, usar_voz=False):
        ...
        self.animar_burbuja_futurista()  
        self.use_gtts = True

    def crear_burbuja_futurista(self):
        w, h = 450, 160
        
        self.bubble_canvas.create_rectangle(0, 0, w, h, fill="#050a14", outline="")
        
        for i in range(20):
            alpha = int(10 + i * 1.5)
            color = f"#{alpha:02x}{alpha+2:02x}{alpha+5:02x}"
        ...
        self.borde_exterior_glow = self.bubble_canvas.create_rectangle(
            8, 13, w-8, h-13,
            outline="#38bdf8", width=1, tags="borde_glow_ext"
        )
        
        self.borde_superior = self.bubble_canvas.create_line(
            15, 15, w-15, 15, 
            fill="#38bdf8", width=4, tags="borde_animado"
        )
        
        self.borde_inferior = self.bubble_canvas.create_line(
            15, h-15, w-15, h-15,
            fill="#22d3ee", width=4, tags="borde_animado"
        )
        
        self.borde_izq = self.bubble_canvas.create_line(
            10, 20, 10, h-20,
            fill="#06b6d4", width=4, tags="borde_animado"
        )
        
        self.borde_der = self.bubble_canvas.create_line(
            w-10, 20, w-10, h-20,
            fill="#0891b2", width=4, tags="borde_animado"
        )
        ...
        for cx, cy in corners:
            ring_outer = self.bubble_canvas.create_oval(
                cx-8, cy-8, cx+8, cy+8,
                outline="#38bdf8", width=2, tags="corner_ring"
            )
            
            ring_mid = self.bubble_canvas.create_oval(
                cx-5, cy-5, cx+5, cy+5,
                fill="#22d3ee", outline="", tags="corner_mid"
            )
            
            center = self.bubble_canvas.create_oval(
                cx-3, cy-3, cx+3, cy+3,
                fill="#ffffff", outline="", tags="corner_center"
            )
            
            for angle in range(0, 360, 90):
                rad = math.radians(angle)
                x_end = cx + 15 * math.cos(rad)
                y_end = cy + 15 * math.sin(rad)
                energy_line = self.bubble_canvas.create_line(
                    cx, cy, x_end, y_end,
                    fill="#38bdf8", width=1, tags="energy_line"
                )
                self.corner_effects.append(energy_line)
            
            self.corner_effects.extend([ring_outer, ring_mid, center])
        ...
        for i in range(30):
            ...
            particle = self.bubble_canvas.create_oval(
                x, y, x+size, y+size,
                fill="#38bdf8", outline="", tags="energy_particle"
            )
        ...
        for i in range(5):
            y = 30 + i * 25
            line = self.bubble_canvas.create_line(
                20, y, w-20, y,
                fill="#334155", width=1, dash=(8, 4), tags="scan_line"
            )
        ...
        for x, y in node_positions:
            ring = self.bubble_canvas.create_oval(
                x-4, y-4, x+4, y+4,
                outline="#10b981", width=2, tags="circuit_node"
            )
            center = self.bubble_canvas.create_oval(
                x-2, y-2, x+2, y+2,
                fill="#10b981", outline="", tags="circuit_node_center"
            )
            self.circuit_nodes.append({'ring': ring, 'center': center, 'x': x, 'y': y})
        ...
        for start_idx, end_idx in connections:
            ...
            line = self.bubble_canvas.create_line(
                ...
                end_pos[0], end_pos[1],
                fill="#1e4d2b", width=1, tags="circuit_line"
            )
            
            # Pulso de energía que viajará por la línea
            pulse = self.bubble_canvas.create_oval(
                start_pos[0]-2, start_pos[1]-2,
                start_pos[0]+2, start_pos[1]+2,
                fill="#10b981", outline="", tags="circuit_pulse"
            )
            
            self.circuit_connections.append({
                'line': line,
                'pulse': pulse,
                'start': start_pos,
                'end': end_pos,
        ...
        base_x, base_y = 30, h - 35
        
        for i in range(3):
            ring = self.bubble_canvas.create_oval(
                base_x + i*20 - 2, base_y - 2,
                base_x + i*20 + 10, base_y + 10,
                outline="#38bdf8", width=1, tags="typing_ring"
            )
            
            dot = self.bubble_canvas.create_oval(
                base_x + i*20, base_y, 
                base_x + i*20 + 8, base_y + 8,
                fill="#64748b", outline="", tags="typing_dot"
            )
            
            shine = self.bubble_canvas.create_oval(
                base_x + i*20 + 2, base_y + 2,
                base_x + i*20 + 4, base_y + 4,
                fill="#94a3b8", outline="", tags="typing_shine"
            )
            
            self.typing_indicators.append({
                'ring': ring,
                'dot': dot,
                'shine': shine
            })
        
        self.icono_bg = self.bubble_canvas.create_oval(
            w - 50, 20, w - 20, 50,
            fill="#1e293b", outline="#38bdf8", width=2, tags="icono_bg"
        )
        ...
        for i in range(3):
            ring = self.bubble_canvas.create_oval(
                w - 48 - i*3, 22 - i*3,
                w - 22 + i*3, 48 + i*3,
                outline="#38bdf8", width=1, tags="icono_ring"
            )
            self.icono_rings.append(ring)
        
        self.icono_pregunta = self.bubble_canvas.create_text(
            w - 35, 35,
            text="?", font=("Segoe UI", 22, "bold"),
            fill="#38bdf8", tags="icono_pregunta"
        )
        
        self.energy_bar_bg = self.bubble_canvas.create_rectangle(
            20, h-20, w-20, h-15,
            fill="#0f172a", outline="#334155", width=1, tags="energy_bar_bg"
        )
        
        self.energy_bar = self.bubble_canvas.create_rectangle(
            20, h-20, 20, h-15,
            fill="#10b981", outline="", tags="energy_bar"
        )
        
        self.energy_bar_glow = self.bubble_canvas.create_rectangle(
            20, h-20, 20, h-15,
            outline="#22d3ee", width=2, tags="energy_bar_glow"
        )
        ...
        for i in range(8):
            ...
            line = self.bubble_canvas.create_line(
                w - 100 + x_offset, y,
                w - 60 + x_offset, y,
                fill="#38bdf8", width=1, dash=(3, 3), tags="hologram"
            )
            self.hologram_lines.append(line)
        
        self.flash_overlay = self.bubble_canvas.create_rectangle(
            10, 15, w-10, h-15,
            fill="#ffffff", outline="", tags="flash", state="hidden"
        )
        
        self.bubble_canvas.tag_raise("burbuja_bg")


    def animar_burbuja_futurista(self):
        try:
            ...
            for i, borde in enumerate([self.borde_superior, self.borde_inferior, 
                ...
                intensity = 120 + int(135 * abs(math.sin(math.radians(self.pulse + i * 90))))
                color = f"#{intensity//4:02x}{intensity:02x}{min(intensity + 60, 255):02x}"
                width = 3 + int(1.5 * abs(math.sin(math.radians(self.pulse + i * 90))))
                self.bubble_canvas.itemconfig(borde, fill=color, width=width)
            
            glow_intensity = 80 + int(100 * abs(math.sin(math.radians(self.pulse))))
            glow_color = f"#{glow_intensity//3:02x}{glow_intensity:02x}{min(glow_intensity + 40, 255):02x}"
            glow_width = 1 + int(2 * abs(math.sin(math.radians(self.pulse))))
            self.bubble_canvas.itemconfig(self.borde_exterior_glow, outline=glow_color, width=glow_width)
            
            for i in range(0, len(self.corner_effects), 7): 
                if i < len(self.corner_effects) - 2:
                    scale = 1 + 0.2 * math.sin(math.radians(self.pulse + i * 30))
                    intensity = 100 + int(155 * abs(math.sin(math.radians(self.pulse + i * 45))))
                    color = f"#{intensity//4:02x}{intensity:02x}{min(intensity + 50, 255):02x}"
                    
                    for j in range(4):
                        if i + j < len(self.corner_effects):
                            self.bubble_canvas.itemconfig(
                                self.corner_effects[i + j],
                                fill=color if j < 4 else "#ffffff"
                            )
            
            for particle in self.energy_particles:
                orbit_x = 3 * math.cos(math.radians(self.pulse + particle['phase']))
                orbit_y = 3 * math.sin(math.radians(self.pulse + particle['phase']))
                
                x = particle['base_x'] + orbit_x
                y = particle['base_y'] + orbit_y
                size = particle['size']
                ...
                alpha = int(100 + 155 * abs(math.sin(math.radians(self.pulse + particle['phase']))))
                color = f"#{alpha//4:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                self.bubble_canvas.itemconfig(particle['id'], fill=color)
            
            for i, line in enumerate(self.scan_lines):
                offset = int(self.pulse / 3) % 12
                alpha = int(50 + 50 * abs(math.sin(math.radians(self.pulse + i * 72))))
                color = f"#{alpha:02x}{alpha+10:02x}{alpha+20:02x}"
                self.bubble_canvas.itemconfig(line, fill=color, dashoffset=offset)
            
            for i, node in enumerate(self.circuit_nodes):
                scale = 1 + 0.15 * math.sin(math.radians(self.pulse + i * 60))
                intensity = 80 + int(100 * abs(math.sin(math.radians(self.pulse + i * 60))))
                color = f"#{intensity//6:02x}{intensity:02x}{intensity//3:02x}"
                
                self.bubble_canvas.itemconfig(node['ring'], outline=color, width=int(2 * scale))
                self.bubble_canvas.itemconfig(node['center'], fill=color)
            
            for connection in self.circuit_connections:
                connection['progress'] = (connection['progress'] + 0.02) % 1.0
                
                start = connection['start']
                end = connection['end']
                progress = connection['progress']
                
                pulse_x = start[0] + (end[0] - start[0]) * progress
                pulse_y = start[1] + (end[1] - start[1]) * progress
                
                pulse_size = 2 + 2 * math.sin(progress * math.pi)
                ...
                pulse_alpha = int(100 + 155 * math.sin(progress * math.pi))
                pulse_color = f"#{pulse_alpha//6:02x}{pulse_alpha:02x}{pulse_alpha//3:02x}"
                self.bubble_canvas.itemconfig(connection['pulse'], fill=pulse_color)
            
            for i, indicator in enumerate(self.typing_indicators):
                offset = 4 * math.sin(math.radians(self.pulse + i * 120))
                coords = self.bubble_canvas.coords(indicator['dot'])
                if coords:
                    base_x = 30 + i*20
                    base_y = 125 + offset
                    
                    self.bubble_canvas.coords(indicator['dot'], 
                        base_x, base_y,
                        base_x + 8, base_y + 8
                    )
                    
                    self.bubble_canvas.coords(indicator['ring'],
                        base_x - 2, base_y - 2,
                        base_x + 10, base_y + 10
                    )
                    
                    self.bubble_canvas.coords(indicator['shine'],
                        base_x + 2, base_y + 2,
                        base_x + 4, base_y + 4
                    )
                
                alpha = int(120 + 135 * abs(math.sin(math.radians(self.pulse + i * 120))))
                dot_color = f"#{alpha//3:02x}{alpha//2:02x}{alpha:02x}"
                ring_color = f"#{alpha//4:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                
                self.bubble_canvas.itemconfig(indicator['dot'], fill=dot_color)
                self.bubble_canvas.itemconfig(indicator['ring'], outline=ring_color)
            
            scale = 1 + 0.2 * math.sin(math.radians(self.pulse))
            ...
            for i, ring in enumerate(self.icono_rings):
                ring_scale = 1 + (0.1 * (i + 1)) * abs(math.sin(math.radians(self.pulse + i * 120)))
                alpha = int(80 + 100 * abs(math.sin(math.radians(self.pulse + i * 120))))
                color = f"#{alpha//4:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                self.bubble_canvas.itemconfig(ring, outline=color)
            
            energy_width = 50 + int(380 * abs(math.sin(math.radians(self.pulse * 0.5))))
            self.bubble_canvas.coords(self.energy_bar, 20, 145, 20 + energy_width, 150)
            self.bubble_canvas.coords(self.energy_bar_glow, 20, 145, 20 + energy_width, 150)
            
            if energy_width < 150:
                bar_color = "#ef4444"
                glow_color = "#f87171"
            elif energy_width < 300:
                bar_color = "#f59e0b"
                glow_color = "#fbbf24"
            else:
                bar_color = "#10b981"
            ...
            self.bubble_canvas.itemconfig(self.energy_bar, fill=bar_color)
            self.bubble_canvas.itemconfig(self.energy_bar_glow, outline=glow_color, width=2)
            
            for i, line in enumerate(self.hologram_lines):
                offset = int(self.pulse / 5) % 6
                alpha = int(60 + 80 * abs(math.sin(math.radians(self.pulse + i * 45))))
                color = f"#{alpha//4:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                self.bubble_canvas.itemconfig(line, fill=color, dashoffset=offset)
            
            if self.pulse % 360 < 10: 
                self.bubble_canvas.itemconfig(self.flash_overlay, state="normal")
                opacity = (10 - (self.pulse % 360)) / 10
                self.bubble_canvas.after(50, lambda: self.bubble_canvas.itemconfig(
                    self.flash_overlay, state="hidden"
                ))
            
            self._bubble_anim_job = self.canvas.after(50, self.animar_burbuja_futurista)
...
class SistemaExpertoUI:
    ...
    def finalizar_diagnostico(self):
        ...
        self.agregar_historial("Ejecutando motor de inferencia...")
    ...
    def crear_interfaz(self):
        ...
        self.header_bg_canvas = tk.Canvas(header_frame, bg="#1e293b", highlightthickness=0)
        self.header_bg_canvas.place(x=0, y=0, relwidth=1, relheight=1)
        
        self.header_particles = []
        for _ in range(15):
            self.header_particles.append({
        ...
        self.header_lines = []
        for i in range(3):
            line = self.header_bg_canvas.create_line(
                0, 30 + i*20, 1400, 30 + i*20,
                fill="#334155", width=1, dash=(10, 5)
            )
        ...
        self.title_shadow = tk.Label(title_frame,
                                    text="🔧 Sistema Experto de Diagnóstico",
                                    font=("Segoe UI", 28, "bold"),
                                    bg="#1e293b",
                                    fg="#1a1a2e")
        self.title_shadow.place(x=2, y=17)
        ...
        self.subtitle_shadow = tk.Label(title_frame,
                                        text="Diagnóstico inteligente de problemas de Hardware",
                                        font=("Segoe UI", 12),
                                        bg="#1e293b",
                                        fg="#1a1a2e")
        self.subtitle_shadow.place(x=2, y=57)
        ...
        for btn_config in botones_config:
            btn_visual = self.crear_boton_control_mejorado(
                btn_config["x"], 30, 
                btn_config["icono"], 
                btn_config["color"],
                btn_config["comando"]
        ...
        for i, btn_config in enumerate(botones_config):
            indicador_glow = self.control_canvas.create_oval(
                btn_config["x"] - 5, 50,
                btn_config["x"] + 5, 60,
            ...
            self.indicadores.append({"main": indicador, "glow": indicador_glow})
        ...
        left_panel = self.crear_panel_izquierdo_mejorado(main_container)
        ...
        right_panel = self.crear_panel_derecho_mejorado(main_container)
    ...
    def crear_panel_izquierdo_mejorado(self, main_container):
        """Panel izquierdo con efectos de partículas y bordes animados"""
        
        left_panel = tk.Frame(main_container, bg="#1e293b", width=300)
        left_panel.pack(side="left", fill="both", padx=(0, 10))
        left_panel.pack_propagate(False)

        self.left_bg_canvas = tk.Canvas(left_panel, bg="#0f172a", highlightthickness=0)
        self.left_bg_canvas.place(x=0, y=0, relwidth=1, relheight=1)
        
        for i in range(50):
            alpha = int(30 - (i * 0.6))
            color = f"#{alpha:02x}{alpha+10:02x}{alpha+20:02x}"
            self.left_bg_canvas.create_rectangle(
                0, i, 300, i+1,
                fill=color, outline=""
            )
        ...
        self.left_deco_lines = []
        for i in range(5):
            x = 40 + i * 50
            line = self.left_bg_canvas.create_line(
                x, 0, x, 800,
                fill="#1e293b", width=1, dash=(5, 10)
            )
            self.left_deco_lines.append(line)

        content_frame = tk.Frame(left_panel, bg="#1e293b")
        content_frame.pack(fill="both", expand=True)

        title_container = tk.Frame(content_frame, bg="#1e293b", height=45)
        title_container.pack(fill="x", pady=(10, 5))
        title_container.pack_propagate(False)
        
        title_canvas = tk.Canvas(title_container, bg="#1e293b", 
                                height=45, highlightthickness=0)
        title_canvas.pack(fill="both", expand=True, padx=10)

        self.left_title_border_top = title_canvas.create_line(
            10, 3, 280, 3,
            fill="#38bdf8", width=2
        )
        ...
        self.left_title_border_bottom = title_canvas.create_line(
            10, 41, 280, 41,
            fill="#22d3ee", width=2
        )
        ...
        for x in [10, 280]:
            for y in [3, 41]:
                title_canvas.create_oval(
                    x-2, y-2, x+2, y+2,
                    fill="#38bdf8", outline=""
                )

        comp_container = tk.Frame(content_frame, bg="#0f172a")
        comp_container.pack(fill="both", expand=True, padx=10, pady=(5, 10))

        self.comp_border_canvas = tk.Canvas(comp_container, bg="#0f172a", 
                                            highlightthickness=0)
        self.comp_border_canvas.pack(fill="both", expand=True)

        def actualizar_bordes_izq(event=None):
            try:
                w = self.comp_border_canvas.winfo_width()
                h = self.comp_border_canvas.winfo_height()
                
                self.comp_border_canvas.coords(self.comp_borders['top'], 5, 5, w-5, 5)
                self.comp_border_canvas.coords(self.comp_borders['right'], w-5, 5, w-5, h-5)
                self.comp_border_canvas.coords(self.comp_borders['bottom'], w-5, h-5, 5, h-5)
                self.comp_border_canvas.coords(self.comp_borders['left'], 5, h-5, 5, 5)
                
                self.comp_canvas.place(x=10, y=10, width=w-20, height=h-20)
            except:
                pass

        self.comp_borders = {
            'top': self.comp_border_canvas.create_line(5, 5, 275, 5, fill="#38bdf8", width=2),
            'right': self.comp_border_canvas.create_line(275, 5, 275, 545, fill="#22d3ee", width=2),
            'bottom': self.comp_border_canvas.create_line(275, 545, 5, 545, fill="#6366f1", width=2),
            'left': self.comp_border_canvas.create_line(5, 545, 5, 5, fill="#8b5cf6", width=2)
        }

        self.comp_canvas = tk.Canvas(self.comp_border_canvas, bg="#0f172a", 
                                    highlightthickness=0)
        self.comp_canvas.place(x=10, y=10, width=260, height=530)

        self.comp_border_canvas.bind("<Configure>", actualizar_bordes_izq)

        self.root.after(100, actualizar_bordes_izq)
        
        self.animar_panel_izquierdo()
        
        return left_panel

    def crear_panel_derecho_mejorado(self, main_container):
      
        right_panel = tk.Frame(main_container, bg="#1e293b", width=300)
        right_panel.pack(side="left", fill="both", padx=(10, 0))
        right_panel.pack_propagate(False)

        self.right_bg_canvas = tk.Canvas(right_panel, bg="#0f172a", highlightthickness=0)
        self.right_bg_canvas.place(x=0, y=0, relwidth=1, relheight=1)
        
        for i in range(50):
            alpha = int(30 - (i * 0.6))
            color = f"#{alpha:02x}{alpha+10:02x}{alpha+20:02x}"
            self.right_bg_canvas.create_rectangle(
                0, i, 300, i+1,
                fill=color, outline=""
            )
        ...
        self.right_hex_pattern = []
        for row in range(15):
            for col in range(5):
                x = 30 + col * 60 + (row % 2) * 30
                y = 50 + row * 52
                hex_lines = self.crear_hexagono(x, y, 25)
                self.right_hex_pattern.extend(hex_lines)

        content_frame = tk.Frame(right_panel, bg="#1e293b")
        content_frame.pack(fill="both", expand=True)

        title_container = tk.Frame(content_frame, bg="#1e293b", height=60)
        ...
        title_canvas.create_text(
            145, 30,
            text="Historial de Diagnóstico",
            font=("Segoe UI", 14, "bold"),
            fill="#10b981"
        )

        history_container = tk.Frame(content_frame, bg="#0f172a")
        history_container.pack(fill="both", expand=True, padx=10, pady=10)
        
        self.history_border_canvas = tk.Canvas(history_container, bg="#0f172a", 
                                            highlightthickness=0)
        self.history_border_canvas.pack(fill="both", expand=True)

        def actualizar_bordes_der(event=None):
            try:
                w = self.history_border_canvas.winfo_width()
                h = self.history_border_canvas.winfo_height()
                
                self.history_border_canvas.coords(self.history_borders['top'], 5, 5, w-5, 5)
                self.history_border_canvas.coords(self.history_borders['right'], w-5, 5, w-5, h-5)
                self.history_border_canvas.coords(self.history_borders['bottom'], w-5, h-5, 5, h-5)
                self.history_border_canvas.coords(self.history_borders['left'], 5, h-5, 5, 5)
            except:
                pass

        self.history_scan_line = self.history_border_canvas.create_line(
            0, 0, 280, 0,
            fill="#10b981", width=2
        )

        self.history_borders = {
            'top': self.history_border_canvas.create_line(5, 5, 275, 5, fill="#10b981", width=2),
            'right': self.history_border_canvas.create_line(275, 5, 275, 545, fill="#22d3ee", width=2),
            'bottom': self.history_border_canvas.create_line(275, 545, 5, 545, fill="#3b82f6", width=2),
            'left': self.history_border_canvas.create_line(5, 545, 5, 5, fill="#8b5cf6", width=2)
        }

        history_frame = tk.Frame(self.history_border_canvas, bg="#0f172a")
        history_frame.place(x=10, y=10, width=260, height=530)

        self.history_border_canvas.bind("<Configure>", actualizar_bordes_der)

        self.root.after(100, actualizar_bordes_der)
        ...
        self.animar_panel_derecho()
        
        return right_panel

    def crear_hexagono(self, x, y, size):
        hexlines = []
        for i in range(6):
            ...
            line = self.right_bg_canvas.create_line(
                x1, y1, x2, y2,
                fill="#1e293b", width=1
            )
    ...
    def animar_panel_izquierdo(self):
        try:
            if not self.root.winfo_exists():
                return
            
            self.left_bg_canvas.delete("particle")
            for particle in self.left_particles:
                particle['y'] -= particle['speed']
                if particle['y'] < 0:
                    particle['y'] = 800
                    particle['x'] = random.randint(0, 300)
                
                alpha = int(particle['alpha'] + 50 * math.sin(math.radians(self.pulse)))
                alpha = max(30, min(alpha, 200))
                color = f"#{alpha//3:02x}{alpha//2:02x}{alpha:02x}"
                
                self.left_bg_canvas.create_oval(
                    particle['x'], particle['y'],
                    particle['x'] + particle['size'],
                    particle['y'] + particle['size'],
            ...
            for i, line in enumerate(self.left_deco_lines):
                offset = int(self.pulse / 5) % 15
                self.left_bg_canvas.itemconfig(line, dashoffset=offset + i*3)
            
            for border_name, border_id in self.comp_borders.items():
                intensity = 100 + int(155 * abs(math.sin(math.radians(self.pulse))))
                color = f"#{intensity//4:02x}{intensity:02x}{min(intensity + 50, 255):02x}"
                self.comp_border_canvas.itemconfig(border_id, fill=color)
            
            self.root.after(50, self.animar_panel_izquierdo)
            
        except (tk.TclError, RuntimeError):
            pass

    def animar_panel_derecho(self):
        try:
            if not self.root.winfo_exists():
                return
            
            self.right_bg_canvas.delete("particle")
            for particle in self.right_particles:
                ...
                alpha = int(particle['alpha'] + 50 * math.sin(math.radians(self.pulse + 180)))
                alpha = max(30, min(alpha, 200))
                color = f"#{alpha//4:02x}{alpha:02x}{alpha//3:02x}"
                
                self.right_bg_canvas.create_oval(
                    particle['x'], particle['y'],
                    particle['x'] + particle['size'],
                    particle['y'] + particle['size'],
                    fill=color, outline="", tags="particle"
            ...
            for i, hex_line in enumerate(self.right_hex_pattern):
                alpha = int(30 + 20 * math.sin(math.radians(self.pulse + i * 10)))
                color = f"#{alpha:02x}{alpha+5:02x}{alpha+10:02x}"
                self.right_bg_canvas.itemconfig(hex_line, fill=color)
            
            scan_y = (self.pulse * 2) % 550
            self.history_border_canvas.coords(self.history_scan_line, 5, scan_y, 275, scan_y)
            
            for border_name, border_id in self.history_borders.items():
                intensity = 100 + int(155 * abs(math.sin(math.radians(self.pulse + 90))))
                color = f"#{intensity//4:02x}{intensity:02x}{intensity//3:02x}"
                self.history_border_canvas.itemconfig(border_id, fill=color)
            
            self.root.after(50, self.animar_panel_derecho)
            
        except (tk.TclError, RuntimeError):
            pass

    def crear_boton_control_mejorado(self, x, y, icono, color, comando):
        anillo_exterior = self.control_canvas.create_oval(
            x - 25, y - 25, x + 25, y + 25,
            outline=color, width=1
        )
        
        sombra = self.control_canvas.create_oval(
            x - 21, y - 19, x + 23, y + 23,
            fill="#0a0f1a", outline="", tags=f"btn_{id(icono)}"
        )
        ...
        brillo_top = self.control_canvas.create_arc(
            x - 18, y - 18, x + 18, y + 18,
            start=45, extent=90,
            fill=self.ajustar_brillo(color, 1.4),
            outline="", tags=f"btn_{id(icono)}"
        )
        ...
        particulas = []
        for i in range(4):
            particula = self.control_canvas.create_oval(
                x - 2, y - 2, x + 2, y + 2,
        ...
        def on_enter(e):
            ...
            self.control_canvas.itemconfig(anillo_exterior, width=2, outline=self.ajustar_brillo(color, 1.3))
            self.control_canvas.itemconfig(circulo_borde, width=3)
            
            self.control_canvas.coords(anillo_exterior, x - 28, y - 28, x + 28, y + 28)
            self.control_canvas.config(cursor="hand2")

        def on_leave(e):
            self.control_canvas.itemconfig(circulo_fondo, fill=color)
            self.control_canvas.itemconfig(anillo_exterior, width=1, outline=color)
            self.control_canvas.itemconfig(circulo_borde, width=2)
            self.control_canvas.coords(anillo_exterior, x - 25, y - 25, x + 25, y + 25)
        ...
        def on_click(e):
            for i in range(3):
                onda = self.control_canvas.create_oval(
                    x - 5, y - 5, x + 5, y + 5,
                    outline=self.ajustar_brillo(color, 1.5), width=2
                )
                self.animar_onda_control(onda, x, y, i * 100)
    ...
    def animar_header(self):
        try:
            ...
            self.header_bg_canvas.delete("particle")
            for particle in self.header_particles:
                ...
                color = f"#{alpha//2:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                
                self.header_bg_canvas.create_oval(
                    particle['x'], particle['y'],
                    particle['x'] + particle['size'],
                    particle['y'] + particle['size'],
                    fill=color, outline="", tags="particle"
            ...
            for i, line in enumerate(self.header_lines):
                offset = 5 * math.sin(math.radians(self.pulse + i * 120))
                dash_offset = int(self.pulse / 10) % 15
                self.header_bg_canvas.itemconfig(
                    line, 
                    dash=(10 + int(offset), 5),
                    dashoffset=dash_offset
                )
            
            for btn in self.botones_control:
                for i, particula in enumerate(btn['particulas']):
                    angle = (self.angle + i * 90) % 360
                    dist = 30
                    px = btn['x'] + dist * math.cos(math.radians(angle))
                    py = btn['y'] + dist * math.sin(math.radians(angle))
                    size = 1 + math.sin(math.radians(self.pulse + i * 90))
            ...
            for i, indicador in enumerate(self.indicadores):
                color_actual = self.control_canvas.itemcget(indicador["main"], 'fill')
                if color_actual == "#10b981": 
                    glow_size = 2 + math.sin(math.radians(self.pulse + i * 120))
                    btn_x = self.botones_control[i]['x']
                    
                    self.control_canvas.coords(
                        indicador["glow"],
                        btn_x - (5 + glow_size), 50,
                        btn_x + (5 + glow_size), 60
                    )
                    
                    alpha = int(100 + 100 * math.sin(math.radians(self.pulse + i * 120)))
                    glow_color = f"#{alpha//4:02x}{alpha:02x}{min(alpha + 50, 255):02x}"
                    self.control_canvas.itemconfig(indicador["glow"], outline=glow_color)
            
            self.root.after(50, self.animar_header)
            
        except (tk.TclError, RuntimeError):
            pass


    def animar_onda_control(self, onda, x, y, delay=0):
        def expandir(size=5):
            if size < 40:
                ...
                width = max(1, int(3 * (1 - size/40)))
                self.control_canvas.itemconfig(onda, width=width)
                self.control_canvas.after(20, lambda: expandir(size + 3))
            else:
                self.control_canvas.delete(onda)
        
        if delay > 0:
            self.control_canvas.after(delay, expandir)
        else:
            expandir()
    ...
    def crear_boton_grande(self, x, y, texto, color, color_acento, comando):
        ...
        def on_enter(e):
            if not btn_state["habilitado"] or btn_state["procesando"]:
                return
            try:
                if self.btn_canvas.winfo_exists():
        ...
        def on_click(e):
            ...
            try:
                if self.btn_canvas.winfo_exists():
                    ...
                    self.btn_canvas.after(100, ejecutar_y_restaurar)
            except (tk.TclError, RuntimeError):
                btn_state["procesando"] = False
    ...
    def actualizar_indicador(self, indice, activo):
        if indice < len(self.indicadores):
            ...
            glow_color = "#22d3ee" if activo else "#334155"
            
            self.control_canvas.itemconfig(self.indicadores[indice]["main"], fill=color)
            self.control_canvas.itemconfig(
                self.indicadores[indice]["glow"], 
                outline=glow_color,
                width=2 if activo else 1
            )
